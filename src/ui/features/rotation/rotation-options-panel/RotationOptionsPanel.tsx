...
import { rotationKey } from "../../../../i18n/contentKeys"
        ...
        <Select
          ...
          options={options.map((option) => ({
            ...
            label:
              ...
              (option.isClassDefault ? t("rotation.options.default") : ""),
          ...
          return (
            <li key={option.id}>
              {options[index - 1]?.group !== option.group && (
                <div className="section-label">{t(GROUP_KEYS[option.group])}</div>
              )}
              <OptionTile
                label={option.name ? t(rotationKey(option.id), option.name) : t("common.unnamed")}
                headMeta={option.isClassDefault ? t("common.default") : undefined}
                detail={
                  optionDps === undefined
                    ? "—"
                    : isSelected
                      ? `${Math.round(optionDps).toLocaleString("en-US")} ${t("common.dps")}`
                      : `${signedDps(delta)} ${t("common.dps")}`
                }
