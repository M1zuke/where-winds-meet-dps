...
//
// Everything else — `schools.json`, the panel-stat table, the conditional-rule
// tables, the inner-way defs, the site param map, saved profiles — refers to the
// `id`. Renaming an inner way is a one-line change here; before this, a rename
// silently broke its stats, its buff param, its context scalars and its
// mechanic gate, with nothing to catch it.
//
...
export const INNER_WAYS: readonly InnerWay[] = [
...
const byId = new Map(INNER_WAYS.map((way) => [way.id, way]))
const byName = new Map(INNER_WAYS.map((way) => [way.name, way]))
